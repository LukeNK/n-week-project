---
layout: draft
title: Cells
---

<!-- This chapter requires A LOT of drawings, so rather than publish everything all at once, just work through it slowly -->

### Prokaryotic cell
Prokaryotic cells are tiny cells are mostly 1–4 micrometers in length. They mostly live independently (**unicellular**) or collaborate with each other (**colonial**). Colonial means that the cells are mostly similar to each other but by staying together, they ensure a higher chance of survival. Bacteria and archaea are structually prokaryotic.

<figure>
    <img src="{{ site.url }}/figures/b/1.png">
    <figcaption>A general prokaryotic cell</figcaption>
</figure>

All prokaryotic cells are surround by **cell walls**. This is a rigid layer that surround the cells. Inside that layer is another softer enclosure called **plasma membrane**. The plasma membrane is constructed of a phospholipid bilayer to create a hydrophobic region that prevents water from entering in while oxygen and certain nutrients can pass through it. However, other special packages must pass through the security gates called **membrane proteins**.

To refer to all of the things inside the cell membrane, biologists called them **cytoplasm**. So first and foremost, the cytoplasm is filled with special cell water (**cytosol**), which allow things to be "floaty" and easily move between places inside a cell. Because of that, all cell functions of a prokaryotic cell occur directly in the cytosol. Even the DNA are just floating around the **nucleoid**, which is basically the name fothe region where the cell's DNA is located.

### Eukaryotic cell
Eukaryotic cells are larger than prokaryotic cell in size because most are about 10–100 micrometers. They can be unicellular or colonial like prokaryotic cells, but they can also work in a **multicellular organisms**. Compared to colonial, the cells here can be specialized in one specific task in a bigger organism. That also means that they are highly dependent on each other and cannot survive on their own. All creatures in the Eukarya domain have eykaryotic cells.

<figure>
    <img src="{{ site.url }}/figures/b/2.png">
    <figcaption>A general eukaryotic cell</figcaption>
</figure>

All eukaryotic cell also have **cell membrane**, but only some cells have **cell walls** (such as plants or fungi). So for the cells without cell walls, you can imagine they are like water baloons being put into a big box.

In contrast to prokaryotic cells, the cell functions are **compartmentalized** where there are **organelles** to do a specific tasks. In this arrangement, the DNA is located inside the **nucleus**, which is basically a room warped by double-membrane.

### Cell components
Most of the components are specialized in one specific function. Moreover, only eukaryotic cells have **organelles**, which are components that have double-membrane to make sure their "tools" will stay within themselves.

#### Nucleus
The nucleus is where the genes are mostly located (some genes are also located in mitochondria and chloroplasts). It is composed of three parts:
- **Nuclear envelope**: basically the double-membrane layer;
- **Nucleolus**: the structure that responsible for ribosome production; and
- **Chromatin**: the DNA and proteins.

It is quite fitting because the chromatin is a blueprint for protein production, so the nucleolus must be the "master" to train students that can read that blueprint.

#### Ribosome
In this list, ribosome is the only component that is NOT considered as an organelle because it is not membrane-bounded. Therefore, it also exists in prokaryotic cells. It is made of ribosomal RNA and protein, yet it's job is to make protein (**protein synthesis**).

There are **free ribosomes**, which is called like that because they are simply floating around in the cytosol. The protein make by them mostly stay within the cell for _domestic consumption_. There are also **bound ribosomes**, which are identical to free ribosomes except they are attached to the rough endoplasmic reticulum. These ribosomes will make proteins for adding to the cell membrane, for packaging, or for exporting.

#### Endoplasmic recticulum
This is a network of sacs and tubes which are continuously connected. It has two different regions: rough and smooth.

**Rough endoplasmic reticulum** has little robosomes attached to it, so that is why it is rough. It has the job to:
- produce protein (thanks to the bound ribosomes);
- send protein to Golgi apparatus; and
- make new membrane for the cell.

**Transitional endoplasmic reticulum** is the depot<note>It is not a distinct region but rather attached to the rough endoplasmic reticulum</note> where protein — after being produced by the rough endoplasmic reticulum — are packaged inside **transport vesicles**. These vesicles are made by having the trasitional endoplasmic reticulum create bubbles with the proteins packed inside them.

**Smooth endoplasmic reticulum** is usually smaller than the rough one, but it is there to:
- produce lipids;
- process carbonhydrates;
- detoxify the cell; and
- store calcium.

#### Golgi apparatus
This organnel is made of many flatten bubbles stacked onto each other. It has two sides:
- **cis face**: the arrival side facing to the nucleus where it receives vesicles from the endoplasmic reticulum; and
- **trans face**: the departure side where it ship packages to other places.

The Golgi apparatus functions like a final packaging depot before the products reach the customer. The products are sometimes finalized here (**cisternal maturation**); other times, certain macromolecules are made here. After the products reach the _trans_ face, the Golgi apparatus sort them into small bubbles and send them into other parts of the cell — sometimes back to the endoplasmic reticulum. They put lables on these vesicles by adding molecular identification tags or exterior molecules that can only dock to the correct organelle.

#### Lysosome
The lysosome has a lot of hydrolytic enzymes used to digest macromolecules. Their main function is to digest food particles in a process called **phagocytosis**. However, they also _recycle_<note>This is a better word for "eat other dead organelles"</note> cell's own organic material in a process called **autophagy**. This recycle process ensures than the cell keep renewing itself rather than wasting resources to make new biological materials.

#### Vacuole
Valcoules are found in every cells. However, they function differently depend on the type of cell. For example, **food vacuolses** are a type of "transport vehicle" to bring food particles to the lysosome during phagocytosis. In many unicellular eukaryotes living in fresh water, they have **contractile vacuoles** to pump excess water out of the cell. The most well-knowwn type of vacuoles are **central vacuoles**, which are commonly found in plants cell. Rather being a small bubble like other types, central vacuoles are usually take up most of the space in plant cells — it pushes most of the cytoplasm close to the cell wall, allowing a cell to grow without requiring too much cytoplasm. It is also "cheaper" than cytosol because it also acts a storage for cell wastes, toxins, nutrients, organic compound, and water.

#### Mitochondrion
The plural form of mitochondrion is "mitochondria" and we all heard the famous phrase "mitochondria are the powerhouses of the cell". However, we would love to know more than that.

Structurally, it is different from other organelles because it is warped in two phospholipid bilayers (so four layers in total). The outer bilayer is smooth but the inside bilayer are folded with **cristae**. These folds help increase the surface area with the inner mechanisms. It also has its own list of embedded protein on its membrane.

The space between the outer bilayer and the inner bilayer is called the **intermembrane space**. Inside the mitochondrion is the **mitochondrial matrix** where there are free ribosomes as well as mitochondrial DNA. However, the main workers in this powerhouse are the proteins and enzymes that makes ATP, and they are attached into the inner membrane.

#### Chloroplast
This is an organelle that only exists in cells that conduct photosynthesis (mostly plants cells). Similar to the mitochondrion, it also have two phosolipid bilayers. The difference is that both chloroplast's bilayers are smooth because you need to have the flattest surface in order to receive the most light possible.

Inside the chloroplasts is a system of connected sacs, and each sac (**thylakoids**) is shaped like a bun; they are usually stacked on top of each other, so biologist called each stack a **granum** (plural is **grana**). The chloroplasts are filled with **stroma**, which is a fluid contains chloroplast DNA, ribosomes, and enzymes.

The main function of the chloroplast is to convert light into chemical "fuel" in a process called **photosynthesis**. However, then can also remove nitrogen and make fatty acids, hormones, aino acids, heme, and vitaims. This is a bit outside of the topic, but it is thought that both the mitochondrion and the chloroplast used to be a bacteria that have a mutualism relationship with the cells. Eventually, those bacterias become a part of the cell. Isn't that cute?

#### Cytoskeleton

#### Cell wall

#### Plasmodesmata

<!-- ### Plant cells and animal cells -->